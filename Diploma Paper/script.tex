...
\usepackage{lscape} % для поворота страницы
\usepackage{longtable} % для длинных таблиц
\usepackage{verbatim} % для многострочных комментарией
\usepackage{amsthm} % для оформления доказательств
\usepackage{algorithm, algpseudocode} % для оформления псевдо кода
\usepackage{cancel} % для обормления сокращений в выражениях
...
\newtheorem{lemma}{Лемма} % для определения лемм
\newtheorem{uncertainty}{Гипотеза}  % для определения гипотеза
...
\newcommand{\setval}[3]{\shortstack{$#1$#2 \\ ($#3$)}} % для отображения сводных таблиц
...
\begin{document} 
	...
	% Титульный лист
	\input{./source/source_tex/title_page.tex}
	\newpage
	...
	\input{./source/source_tex/introduction.tex}
	\input{./source/source_tex/pre_experiment.tex}
	\input{./source/source_tex/experiment.tex}
	\input{./source/source_tex/discussion.tex}
	\input{./source/source_tex/outroduction.tex}
